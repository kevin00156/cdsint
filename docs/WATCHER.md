...
成功失敗看本體的回傳值（SPEC D11）。本體每一條路都要回一個 `engine/entry.py` 的 `result()`：
`ok` 是判決，`summary` 是給人看的一句話（`ok` 是 false 時它同時是呼叫端拿到的錯誤訊息），
`data` 是這個命令自己的計數。回傳 `None`（也就是有一條路忘了回）算失敗，訊息說「returned no
result」，因為那正是「安靜地什麼都沒做卻回報成功」的形狀。

訊息等級不再影響判決，所以 `warning` 可以拿來講一件值得注意但不中止的事。
