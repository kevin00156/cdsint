...
理由：`.project` 是二進位檔，只有 IDE 行程開得了，所以存在裡面的設定要改就得先弄到一個活著的 IDE，agent 要看設定得起一個 IDE，每個新專案從零開始。這一個事實養出了四條入口（Properties 表格、Settings 視窗、`config` 命令、第一次匯出的對話框）和三個只為它存在的機制（強制 `--sync-dir`、電腦名稱戳記、`config set` 之後再按引擎收尾）。文字檔任何編輯器都能改，不需要 IDE。放專案旁而不放同步資料夾裡，是因為這些設定大多是這台機器的事，不是團隊政策，而且只有這樣同步資料夾本身才能一起搬出來，讓專案屬性那條路整個關掉。舊的 `cds-sync-*` 屬性不讀、不遷移：還沒發布過版本，唯一沒預設值的是同步資料夾，而它本來就有對話框會問。schema 在 `cds/core/settings.py`，一份，兩側共用，CI 測得到。
...
讀檔那一支是唯一的驗證：不認識的鍵、型別不對、`plc` 裡有不認識的字、JSON 壞掉，整個命令拒絕，訊息把 4.4 那張表的每個鍵和預設值原樣印出來。檔案是人手改的，打錯字必然發生，安靜地沒效果比錯誤更糟。
...
**引擎收平行路徑前後，229 個物件，2026-09-07 量的。**跟上面那兩組不能直接相減：機器狀態不同，
而且這一組的 compare 是「在磁碟上改一個 `.st`」而不是「改一個 POU」。它的用途是回答一個問題——
把同一條規則從兩份收成一份，付了多少速度。每格是四次跑掉第一次之後的中位數。

| 操作 | 原廠 3.5.21.40（softplc 副本） | Delta 1.10（Shm 副本） |
|---|---|---|
| export | 22.9 → 24.1 秒 | 23.3 → 21.7 秒 |
| compare | 15.8 → 17.3 秒 | 15.3 → 15.2 秒 |
| build | 25.8 → 25.2 秒 | 30.5 → 30.5 秒 |

**這一組數字有一個陷阱，寫在這裡免得下一個人再踩。**上面那幾格是在不同時間點量的，
而每一次量之前副本與同步資料夾的狀態都不一樣，所以誤差大到可以蓋過要量的東西：
用同一支腳本量一份「只刪過死碼」的程式碼，compare 也會「慢 7.8%」。
比較能拿來判斷的量法是控制起點、新舊碼背對背：每一組都從乾淨的 `.project` 副本、空的同步資料夾、
一次 export、改一個 `.st` 開始。這樣的成對量測做了兩次，方向相反：
第一次舊碼 16.59 秒、新碼 17.63 秒（新碼慢 6.3%），第二次新碼 17.74 秒、舊碼 17.96 秒（新碼快 1.2%）。
兩次之間隔了一個半小時，而同一份舊碼從 16.59 秒漂到 17.96 秒——機器本身的變動就有 8%，
跟要量的東西同一個數量級。所以能講的只有這句：**compare 的差異落在這台機器的量測誤差裡面，
沒有量到可以指認的變慢。**要更確定的話，需要的是同一小時內多對交錯量測，這張工單沒有做。

匯出的 `.st` 與 `.xml` 一個位元組都沒變：兩份副本、229 個物件、每一層做完各量一次，
231 行的路徑加 SHA-256 清單從頭到尾同一個雜湊。
