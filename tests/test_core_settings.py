...
def test_there_are_eleven_of_them():
    # SPEC 4.4 and every refusal message say eleven. A twelfth added without
    # touching either would leave both lying.
    assert len(settings.SCHEMA) == 11
...
def test_plc_takes_the_two_words(tmp_path):
    path = write_file(tmp_path, u'{"plc": ["connect", "download"]}')
    assert settings.read(path) == {"plc": ["connect", "download"]}
